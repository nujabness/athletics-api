...
class EpreuveController{
    ...
    static async updateParticipation(request, response) {
        let status = 200;
        let body = {};
        try {
            let epreuve = await Epreuve.findOne({_id: request.body.epreuveId}).populate('participants.athlete');
            epreuve.participants.forEach((participant) => {
                if (participant.athlete._id == request.body.userId){
                    participant.medaille = request.body.medaille
                    participant.resultat = request.body.resultat
                }
            })
            epreuve.save()
            body = {epreuve, 'message': 'Participation Epreuve'};
        } catch (error) {
            status = 500;
            body = {'message': error.message};
        }
